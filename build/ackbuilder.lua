...
local function assertString(s, i)
	if (type(s) ~= "string") then
		error(string.format("parameter %d must be a string, not a %s", i, type(s)))
	end
end
...
local function map(files, callback)
...
local function filter(files, callback)
	return map(files, function (f) return callback(f) and f or nil end)
end
...
local function shell(...)
	...
	return table.unpack {data, fp:close()}
end
...
loadtarget = function(targetname)
	...
	if not targetname:find("%+") then
		if targets[targetname] ~= nil then
