...
public final class AdfHd {
    ...
    public static int adfDevType(Device dev) {
        if ((dev.size == 512 * 11 * 2 * 80) ||
            (dev.size == 512 * 11 * 2 * 81) ||
            (dev.size == 512 * 11 * 2 * 82) ||
            (dev.size == 512 * 11 * 2 * 83)) {
    ...
    public static void adfDeviceInfo(Device dev) {
        ...
        for (i = 0; i < dev.nVol; i++) {
            ...
            if (vol.volName != null) {
                System.out.printf("%2d :  %7d ->%7d, \"%s\"", i,
                    vol.firstBlock,
                    vol.lastBlock,
                    vol.volName);
            } else {
    ...
    /**
     * @deprecated file-backed storage removed; create Device subclass and pass to {@link #adfMountDev(Device)}
     */
    @Deprecated
    public static Device adfMountDev(String filename, boolean ro) {
        throw new UnsupportedOperationException(
            "file-backed storage removed; create Device subclass and pass to adfMountDev(Device)");
    }
    ...
    public static AdfError adfCreateHd(Device dev, int n, List<Partition> partList) {
        ...
        for (i = 0; i < n; i++) {
            Partition p = partList.get(i);
            Volume vol = AdfDisk.adfCreateVol(dev,
                        p.startCyl,
                        p.lenCyl,
                        p.volName,
                        p.volType);
    ...
    public static AdfError adfWriteRDSKblock(Device dev, BRDSKBlock rdsk) {
        ...
        rdsk.id[0] = 'R'; rdsk.id[1] = 'D'; rdsk.id[2] = 'S'; rdsk.id[3] = 'K';
        rdsk.size = 64;
        rdsk.blockSize = AdfConstants.LOGICAL_BLOCK_SIZE;
        rdsk.badBlockList = -1;

        rdsk.diskVendor[0] = 'A'; rdsk.diskVendor[1] = 'D'; rdsk.diskVendor[2] = 'F';
        rdsk.diskVendor[3] = 'l'; rdsk.diskVendor[4] = 'i'; rdsk.diskVendor[5] = 'b';
        rdsk.diskVendor[6] = ' '; rdsk.diskVendor[7] = ' ';
        byte[] prod = "harddisk.adf    ".getBytes();
        System.arraycopy(prod, 0, rdsk.diskProduct, 0, Math.min(prod.length, rdsk.diskProduct.length));
        rdsk.diskRevision[0] = 'v'; rdsk.diskRevision[1] = '1';
        rdsk.diskRevision[2] = '.'; rdsk.diskRevision[3] = '0';
    ...
    public static AdfError adfWritePARTblock(Device dev, int nSect, BPARTBlock part) {
        ...
        part.id[0] = 'P'; part.id[1] = 'A'; part.id[2] = 'R'; part.id[3] = 'T';
        part.size = 64;
    ...
    public static AdfError adfWriteFSHDblock(Device dev, int nSect, BFSHDBlock fshd) {
        ...
        fshd.id[0] = 'F'; fshd.id[1] = 'S'; fshd.id[2] = 'H'; fshd.id[3] = 'D';
        fshd.size = 64;
    ...
    public static AdfError adfWriteLSEGblock(Device dev, int nSect, BLSEGBlock lseg) {
        ...
        lseg.id[0] = 'L'; lseg.id[1] = 'S'; lseg.id[2] = 'E'; lseg.id[3] = 'G';
        lseg.size = 128;
    ...
    private static void copyRDSK(BRDSKBlock src, BRDSKBlock dst) {
        ...
        System.arraycopy(src.controllerVendor, 0, dst.controllerVendor, 0, src.controllerVendor.length);
        System.arraycopy(src.controllerProduct, 0, dst.controllerProduct, 0, src.controllerProduct.length);
        System.arraycopy(src.controllerRevision, 0, dst.controllerRevision, 0, src.controllerRevision.length);
