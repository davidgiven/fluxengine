...
public final class AdfDisk {
    ...
    public static AdfError adfInstallBootBlock(Volume vol, byte[] code) {
        ...
        if (vol.dev.devType != AdfConstants.DEVTYPE_FLOPDD && vol.dev.devType != AdfConstants.DEVTYPE_FLOPHD) {
    ...
    public static void adfVolumeInfo(Volume vol) {
        ...
        switch (vol.dev.devType) {
            ...
            case AdfConstants.DEVTYPE_HARDDISK:
                System.out.printf("Hard Disk partition : %3.1f KBytes\n",
                    (vol.lastBlock - vol.firstBlock + 1) * 512.0 / 1024.0);
                break;
            case AdfConstants.DEVTYPE_HARDFILE:
                System.out.printf("HardFile : %3.1f KBytes\n",
                    (vol.lastBlock - vol.firstBlock + 1) * 512.0 / 1024.0);
                break;
            default:
        ...
        month = mm[0];
        days = dd[0];
        System.out.printf("created %d/%02d/%02d %d:%02d:%02d\n", days, month, year,
            root.coMins / 60, root.coMins % 60, root.coTicks / 50);
        adfDays2Date(root.days, yy, mm, dd);
        year = yy[0];
        month = mm[0];
        days = dd[0];
        System.out.printf("last access %d/%02d/%02d %d:%02d:%02d,   ", days, month, year,
