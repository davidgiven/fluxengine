...
public final class AdfDir
{
    ...
    public static AdfError adfRenameEntry(
            Volume vol,
            int pSect,
            String oldName,
            int nPSect,
            String newName)
    {
        ...
        intl = AdfConstants.isINTL(vol.dosType & 0xFF) ||
                AdfConstants.isDIRCACHE(vol.dosType & 0xFF);
        ...
        } else
        {
            ...
            {
                rc = AdfFile.adfWriteFileHdrBlock(
                        vol,
                        previous.headerKey,
    ...
    public static AdfError adfRemoveEntry(Volume vol, int pSect, String name)
    {
        ...
        {
            AdfBitm.adfSetBlockFree(vol, nSect);
            /* free dir cache block : the directory must be empty, so there's only one cache
            block */
            if (AdfConstants.isDIRCACHE(vol.dosType & 0xFF))
            {
    ...
    public static int adfNameToEntryBlk(
            Volume vol,
            int[] ht,
            String name,
            BEntryBlock entry,
     ...
     * adfCreateEntry
     *
     * if 'thisSect'==-1, allocate a sector, and insert its pointer into the hashTable of 'dir',
     * using the
    ...
    public static int adfCreateEntry(Volume vol, BEntryBlock dir, String name, int thisSect)
    {
        boolean intl = false;
    ...
    public static AdfError adfCreateFile(
            Volume vol,
            int nParent,
            String name,
    ...
    public static AdfError adfReadEntryBlock(Volume vol, int nSect, BEntryBlock ent)
    {
                ...
                (ent.commLen & 0xFF) > AdfConstants.MAXCMMTLEN)
        {
            ...
            System.out.printf(
                    "nameLen=%d, commLen=%d, name=%s sector%d\n",
                    ent.nameLen & 0xFF,
                    ent.commLen & 0xFF,
                    new String(ent.name, 0, ent.nameLen & 0xFF),
                    ent.headerKey);
        }
