...
public final class AdfFile
{
    ...
    public static void adfFlushFile(File file)
    {
        ...
        if (file.currentExt != null)
        {
            if (file.writeMode)
            {
                adfWriteFileExtBlock(file.volume, file.currentExt.headerKey, file.currentExt);
            }
        }
        ...
        if (file.writeMode)
        {
            ...
            if (AdfConstants.isDIRCACHE(file.volume.dosType & 0xFF))
            {
                ...
                AdfCache.adfUpdateCache(
                        file.volume,
                        parent,
                        fileHeaderToEntryBlock(file.fileHdr),
                        false);
            }
    ...
    public static AdfError adfGetFileBlocks(
            Volume vol,
            BFileHeaderBlock entry,
            FileBlocks fileBlocks)
    {
        ...
        adfFileRealSize(entry.byteSize, vol.datablockSize, nbDataArr, nbExtArr);
        ...
        } else
        {
            ...
            } else
            {
                if (file.nDataBlock == AdfConstants.MAX_DATABLK)
                {
                    ...
                    adfReadFileExtBlock(file.volume, file.fileHdr.extension, file.currentExt);
                    file.posInExtBlk = 0;
                } else if (file.posInExtBlk == AdfConstants.MAX_DATABLK)
                {
                    adfReadFileExtBlock(file.volume, file.currentExt.extension, file.currentExt);
                    file.posInExtBlk = 0;
                }
    ...
    public static int adfWriteFile(File file, int n, ByteBuffer buffer)
    {
        ...
        while (bytesWritten < n)
        {
            ...
            for (int i = 0; i < size; i++)
            {
                file.currentData.put(
                        dataOffset + file.posInDataBlk + i,
                        buffer.get(bytesWritten + i));
            }
            file.pos += size;
            bytesWritten += size;
            file.posInDataBlk += size;
    ...
    public static int adfPos2DataBlock(
            int pos,
            int blockSize,
            int[] posInExtBlk,
            int[] posInDataBlk,
    ...
    private static void entryBlockToFileHeader(BEntryBlock entry, BFileHeaderBlock fhdr)
    {
        ...
        System.arraycopy(
                ...
                fhdr.comment,
                0,
                Math.min(entry.comment.length, fhdr.comment.length));
        ...
        System.arraycopy(
                ...
                fhdr.fileName,
                0,
                Math.min(entry.name.length, fhdr.fileName.length));
        fhdr.r4 = entry.r4;
    ...
    private static BEntryBlock fileHeaderToEntryBlock(BFileHeaderBlock fhdr)
    {
        ...
        System.arraycopy(
                ...
                e.comment,
                0,
                Math.min(fhdr.comment.length, e.comment.length));
        e.days = fhdr.days;
        e.mins = fhdr.mins;
        e.ticks = fhdr.ticks;
        ...
        System.arraycopy(
                ...
                e.name,
                0,
                Math.min(fhdr.fileName.length, e.name.length));
        e.r4 = fhdr.r4;
