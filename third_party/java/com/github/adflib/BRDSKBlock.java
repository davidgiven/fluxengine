...
public final class BRDSKBlock {
    ...
    public static BRDSKBlock read(ByteBuffer buf, int off) {
        ...
        for (int i = 0; i < 6; i++) blk.r1[i] = b.getInt(off + 40 + i * 4);
        ...
        for (int i = 0; i < 8; i++) blk.diskVendor[i] = b.get(off + 0xa0 + i);
        for (int i = 0; i < 16; i++) blk.diskProduct[i] = b.get(off + 0xa8 + i);
        for (int i = 0; i < 4; i++) blk.diskRevision[i] = b.get(off + 0xb8 + i);
        for (int i = 0; i < 8; i++) blk.controllerVendor[i] = b.get(off + 0xbc + i);
        for (int i = 0; i < 16; i++) blk.controllerProduct[i] = b.get(off + 0xc4 + i);
        for (int i = 0; i < 4; i++) blk.controllerRevision[i] = b.get(off + 0xd4 + i);
        for (int i = 0; i < 10; i++) blk.r5[i] = b.getInt(off + 0xd8 + i * 4);
        return blk;
    }
    ...
    public void write(ByteBuffer buf, int off) {
        ...
        for (int i = 0; i < 6; i++) b.putInt(off + 40 + i * 4, r1[i]);
        ...
        for (int i = 0; i < 3; i++) b.putInt(off + 0x54 + i * 4, r2[i]);
        b.putInt(off + 0x60, writePreComp);
        b.putInt(off + 0x64, reducedWrite);
        b.putInt(off + 0x68, stepRate);
        for (int i = 0; i < 5; i++) b.putInt(off + 0x6c + i * 4, r3[i]);
        ...
        for (int i = 0; i < 8; i++) b.put(off + 0xa0 + i, diskVendor[i]);
        for (int i = 0; i < 16; i++) b.put(off + 0xa8 + i, diskProduct[i]);
        for (int i = 0; i < 4; i++) b.put(off + 0xb8 + i, diskRevision[i]);
        for (int i = 0; i < 8; i++) b.put(off + 0xbc + i, controllerVendor[i]);
        for (int i = 0; i < 16; i++) b.put(off + 0xc4 + i, controllerProduct[i]);
        for (int i = 0; i < 4; i++) b.put(off + 0xd4 + i, controllerRevision[i]);
        for (int i = 0; i < 10; i++) b.putInt(off + 0xd8 + i * 4, r5[i]);
    }
