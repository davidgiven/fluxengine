...
public final class BFSHDBlock {
    ...
    public static BFSHDBlock read(ByteBuffer buf, int off) {
        ...
        for (int i = 0; i < 4; i++) blk.dosType[i] = b.get(off + 32 + i);
        ...
        for (int i = 0; i < 23; i++) blk.r2[i] = b.getInt(off + 80 + i * 4);
        for (int i = 0; i < 21; i++) blk.r3[i] = b.getInt(off + 172 + i * 4);
        return blk;
    }
    ...
    public void write(ByteBuffer buf, int off) {
        ...
        for (int i = 0; i < 4; i++) b.put(off + 32 + i, dosType[i]);
        ...
        for (int i = 0; i < 23; i++) b.putInt(off + 80 + i * 4, r2[i]);
        for (int i = 0; i < 21; i++) b.putInt(off + 172 + i * 4, r3[i]);
    }
