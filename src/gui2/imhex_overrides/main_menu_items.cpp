...
namespace hex::plugin::builtin
{
                ...
                [error]
                {
                    switch (error)
                    {
                        case IPSError::InvalidPatchHeader:
                            ui::ToastError::open(
                                "hex.builtin.menu.file.export.ips.popup.invalid_patch_header_error"_lang);
                            break;
                        case IPSError::AddressOutOfRange:
                            ui::ToastError::open(
                                "hex.builtin.menu.file.export.ips.popup.address_out_of_range_error"_lang);
                            break;
                        case IPSError::PatchTooLarge:
                            ui::ToastError::open(
                                "hex.builtin.menu.file.export.ips.popup.patch_too_large_error"_lang);
                            break;
                        case IPSError::InvalidPatchFormat:
                            ui::ToastError::open(
                                "hex.builtin.menu.file.export.ips.popup.invalid_patch_format_error"_lang);
                            break;
                        case IPSError::MissingEOF:
                            ui::ToastError::open(
                                "hex.builtin.menu.file.export.ips.popup.missing_eof_error"_lang);
                            break;
                    }
                });
        }
    ...
    namespace
    {
        ...
        void importIPSPatch()
        {
            fs::openFileBrowser(fs::DialogMode::Open,
                ...
                [](const auto& path)
                {
                    TaskManager::createTask("hex.ui.common.processing",
                        TaskManager::NoProgress,
                        [path](auto& task)
                        {
                            auto patchData =
                                wolv::io::File(path, wolv::io::File::Mode::Read)
                                    .readVector();
                            ...
                            provider->getUndoStack().groupOperations(
                                patch->get().size(),
                ...
                [](const auto& path)
                {
                    TaskManager::createTask("hex.ui.common.processing",
                        ...
                        [path](auto& task)
                        {
                            ...
                            {
                                ui::ToastError::open(
                                    "hex.builtin.menu.file.import.modified_file.popup.invalid_size"_lang);
                                return;
                            }
    ...
    namespace
    {
        ...
        void exportBase64()
        {
            fs::openFileBrowser(fs::DialogMode::Save,
                {},
                [](const auto& path)
                {
                    TaskManager::createTask("hex.ui.common.processing",
                        ...
                        [path](auto&)
                        {
                            wolv::io::File outputFile(
                                path, wolv::io::File::Mode::Create);
                            if (!outputFile.isValid())
                            {
                                TaskManager::doLater(
                                    ...
                                    {
                                        ui::ToastError::open(
                                            "hex.builtin.menu.file.export.error.create_file"_lang);
                                    });
                                return;
                            }
                            ...
                            for (u64 address = 0;
                                address < provider->getActualSize();
                                address += bytes.size())
                            {
                                bytes.resize(std::min<u64>(bytes.size(),
                                    provider->getActualSize() - address));
                                provider->read(
                                    provider->getBaseAddress() + address,
                                    bytes.data(),
                                    bytes.size());
                ...
                [](const auto& path)
                {
                    TaskManager::createTask("hex.ui.common.processing",
                        ...
                        [path](auto& task)
                        {
                            ...
                            auto selection =
                                ImHexApi::HexEditor::getSelection();
                            for (u64 address = selection->getStartAddress();
                                address <= selection->getEndAddress();
                                address += bytes.size())
                            {
                                bytes.resize(std::min<u64>(bytes.size(),
                                    selection->getEndAddress() - address + 1));
                                provider->read(
        ...
        {
            for (const auto& formatter :
                ContentRegistry::DataFormatter::impl::getExportMenuEntries())
            {
                if (menu::menuItem(Lang(formatter.unlocalizedName),
                        Shortcut::None,
                        false,
                        ImHexApi::Provider::isValid() &&
                            ImHexApi::Provider::get()->getActualSize() > 0))
                {
                    fs::openFileBrowser(fs::DialogMode::Save,
                        {},
                        [&formatter](const auto& path)
                        {
                            TaskManager::createTask(
                                "hex.builtin.task.exporting_data",
                                TaskManager::NoProgress,
                                [&formatter, path](auto&)
                                {
                                    auto provider = ImHexApi::Provider::get();
                                    ...
                                    auto result = formatter.callback(provider,
                                    ...
                                    if (!file.isValid())
                                    {
                                        TaskManager::doLater(
                                            ...
                                            {
                                                ui::ToastError::open(
                                                    "hex.builtin.menu.file.export.as_language.popup.export_error"_lang);
                                            });
                                        return;
                                    }
                ...
                [](auto&)
                {
                        ...
                        ImHexApi::Provider::getProviders())
                    {
                        ...
                        for (const auto& generator :
                            ContentRegistry::Reports::impl::getGenerators())
                        {
                    ...
                    TaskManager::doLater(
                        [data]
                        {
                            fs::openFileBrowser(fs::DialogMode::Save,
                                ...
                                [&data](const auto& path)
                                {
                                    auto file = wolv::io::File(
                                        path, wolv::io::File::Mode::Create);
                                    if (!file.isValid())
                                    {
                                        ui::ToastError::open(
                                            "hex.builtin.menu.file.export.report.popup.export_error"_lang);
                                        return;
                                    }
        ...
        void exportIPSPatch()
        {
            ...
            // Make sure there's no patch at address 0x00454F46 because that
            // would cause the patch to contain the sequence "EOF" which signals
            // the end of the patch
            if (!patches->get().contains(0x00454F45) &&
                patches->get().contains(0x00454F46))
            {
                u8 value = 0;
                        ...
                        [data]
                        {
                            fs::openFileBrowser(fs::DialogMode::Save,
                                ...
                                [&data](const auto& path)
                                {
                                    ...
                                    {
                                        ui::ToastError::open(
                                            "hex.builtin.menu.file.export.ips.popup.export_error"_lang);
                                        return;
                                    }
                                    ...
                                    if (data.has_value())
                                    {
                                        ...
                                        EventPatchCreated::post(bytes.data(),
                                            bytes.size(),
                                            PatchKind::IPS);
                                    }
                                    else
        ...
        void exportIPS32Patch()
        {
            ...
            // Make sure there's no patch at address 0x45454F46 because that
            // would cause the patch to contain the sequence "*EOF" which
            // signals the end of the patch
            if (!patches->get().contains(0x45454F45) &&
                patches->get().contains(0x45454F46))
            {
                u8 value = 0;
    ...
    /**
     * @brief returns true if there is a currently selected provider, and it is
     * possibl to dump data from it
     */
    ...
    static void createFileMenu()
    {
        ...
        ContentRegistry::UserInterface::addMenuItem(
            {"hex.builtin.menu.file", "hex.builtin.menu.file.create_file"},
            ICON_VS_FILE,
            1050,
            CTRLCMD + Keys::N + AllowWhileTyping,
            []
            {
                auto newProvider = hex::ImHexApi::Provider::createProvider(
                    "hex.builtin.provider.mem_file", true);
        ...
        /* Open File */
        ContentRegistry::UserInterface::addMenuItem(
            {"hex.builtin.menu.file", "hex.builtin.menu.file.open_file"},
            ICON_VS_FOLDER_OPENED,
            1100,
            CTRLCMD + Keys::O + AllowWhileTyping + ShowOnWelcomeScreen,
            []
            {
                RequestOpenWindow::post("Open File");
            },
            noRunningTasks,
            ContentRegistry::Views::getViewByName(
                "hex.builtin.view.hex_editor.name"));

        /* Open Other */
        ContentRegistry::UserInterface::addMenuItemSubMenu(
            {"hex.builtin.menu.file", "hex.builtin.menu.file.open_other"},
            ICON_VS_TELESCOPE,
            1150,
            []
            {
                for (const auto& [unlocalizedProviderName, icon] :
                    ContentRegistry::Provider::impl::getEntries())
                {
                    if (menu::menuItemEx(Lang(unlocalizedProviderName), icon))
                        ImHexApi::Provider::createProvider(
                            unlocalizedProviderName);
                }
            },
            noRunningTasks);
        ...
        /* Reload Provider */
        ContentRegistry::UserInterface::addMenuItem(
            {"hex.builtin.menu.file", "hex.builtin.menu.file.reload_provider"},
            ICON_VS_REFRESH,
            1250,
            CTRLCMD + Keys::R + AllowWhileTyping,
            []
            {
                auto provider = ImHexApi::Provider::get();
        ...
        {
            ContentRegistry::UserInterface::addMenuItemSubMenu(
                {"hex.builtin.menu.file", "hex.builtin.menu.file.import"},
                ICON_VS_SIGN_IN,
                5140,
                []
                {
            ...
            ContentRegistry::UserInterface::addMenuItem(
                ...
                ICON_VS_GIT_PULL_REQUEST_NEW_CHANGES,
                5150,
                Shortcut::None,
                importIPSPatch,
        ...
        {
            ContentRegistry::UserInterface::addMenuItemSubMenu(
                {"hex.builtin.menu.file", "hex.builtin.menu.file.export"},
                ICON_VS_SIGN_OUT,
                6000,
                []
                {
            ...
            ContentRegistry::UserInterface::addMenuItem(
                {"hex.builtin.menu.file",
                    ...
                    "hex.builtin.menu.file.export.selection_to_file"},
                ICON_VS_FILE_BINARY,
                6010,
                Shortcut::None,
        ...
        ContentRegistry::UserInterface::addMenuItemSeparator(
            {"hex.builtin.menu.file"}, 10000);

        /* Close Provider */
        ContentRegistry::UserInterface::addMenuItem(
            {"hex.builtin.menu.file", "hex.builtin.menu.file.close"},
            ICON_VS_CHROME_CLOSE,
            10050,
            CTRLCMD + Keys::W + AllowWhileTyping,
            []
            {
