...
namespace
{
    ...
    std::string replace(
        std::string string, const std::string& from, const std::string& to)
    {
        if (!from.empty())
            for (size_t pos = 0;
                (pos = string.find(from, pos)) != std::string::npos;
                pos += to.size())
...
// Replace all backslashes with forward slashes on Windows
    ...
    std::vector<std::string> splitString(
        std::string string, const std::string& delimiter)
    {
...
int main(int argc, const char* const argv[])
{
    ...
    for (const auto& resourceLocation : resourceLocations)
    {
        ...
        for (const auto& entry :
            fs::recursive_directory_iterator(resourceLocation))
        {
            ...
            inputData.resize(
                std::fread(inputData.data(), 1, entry.file_size(), file));
            std::fclose(file);
            ...
            bytes.resize(inputData.size() * 1.1 +
                         12); // Slightly larger than the input size
            ...
            outputFile << "static std::array<std::uint8_t, " << bytes.size() + 1
...

    {
        ...
        outputFile
            << "ROMFS_VISIBILITY std::span<romfs::impl::ResourceLocation> "
               "RomFs_" LIBROMFS_PROJECT_NAME "_get_resources() {\n";
        outputFile << "    static std::array<romfs::impl::ResourceLocation, "
                   << identifierCount << "> resources = {\n";
        ...
        for (std::uint64_t i = 0; i < identifierCount; i++)
        {
            ...
            outputFile << "        " << "romfs::impl::ResourceLocation { \""
                       << toPathString(paths[i].string())
                       << "\", romfs::Resource({ "
