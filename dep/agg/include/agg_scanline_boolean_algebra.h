...
namespace agg
{
    ...
    struct sbool_xor_formula_saddle
    {
        ...
        static AGG_INLINE unsigned calculate(unsigned a, unsigned b)
        {
            ...
            a = (cover_mask * cover_mask - (a << cover_shift) + k) >>
                cover_shift;
            b = (cover_mask * cover_mask - (b << cover_shift) + k) >>
                cover_shift;
            ...
            Scanline& sl) const
        {
            ...
            switch ((span1->len < 0) | ((span2->len < 0) << 1))
            {
                ...
                case 1: // span1 is solid, span2 is AA
                    ...
                    {
                        cover =
                            XorFormula::calculate(*(span1->covers), *covers2++);
                        if (cover)
                            sl.add_cell(x, cover);
                        ++x;
                    } while (--len);
                ...
                case 2: // span1 is AA, span2 is solid
                    ...
                    {
                        cover =
                            XorFormula::calculate(*covers1++, *(span2->covers));
                        if (cover)
                            sl.add_cell(x, cover);
                        ++x;
                    } while (--len);
                ...
                case 3: // Both are solid spans
                    cover = XorFormula::calculate(
                        *(span1->covers), *(span2->covers));
                    if (cover)
            ...
            switch ((span1->len < 0) | ((span2->len < 0) << 1))
            {
                ...
                case 2: // span1 is AA, span2 is solid
                    ...
                    if (*(span2->covers) != cover_full)
                    {
                        ...
                        {
                            cover =
                                *covers1++ * (cover_mask - *(span2->covers));
                            if (cover)
                            {
                                sl.add_cell(x,
                                    (cover == cover_full * cover_full)
        ...
        while (flag1 || flag2)
        {
            if (flag1 && flag2)
            {
                if (sl1.y() == sl2.y())
                {
                    ...
                    //----------------------
                    sbool_unite_scanlines(
                        sl1, sl2, sl, add_span1, add_span2, combine_spans);
                    if (sl.num_spans())
                    {
                        sl.finalize(sl1.y());
                        ren.render(sl);
    ...
    void sbool_intersect_shapes_aa(ScanlineGen1& sg1,
        ...
        Renderer& ren)
    {
        sbool_intersect_spans_aa<Scanline1, Scanline2, Scanline>
            combine_functor;
        ...
        Renderer& ren)
    {
        sbool_add_span_bin<Scanline1, Scanline> add_functor1;
        sbool_add_span_bin<Scanline2, Scanline> add_functor2;
        sbool_combine_spans_empty<Scanline1, Scanline2, Scanline>
            combine_functor;
        sbool_unite_shapes(sg1,
            sg2,
            sl1,
