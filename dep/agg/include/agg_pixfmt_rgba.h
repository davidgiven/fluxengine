...
namespace agg
{
    ...
    struct conv_rgba_pre
    {
        ...
        {
            return color_type(
                p[Order::R], p[Order::G], p[Order::B], p[Order::A])
                .demultiply();
        }
    };
    ...
    //=============================================================blender_rgba
    // Blends "plain" (i.e. non-premultiplied) colors into a premultiplied
    // buffer.
    template <class ColorT, class Order>
    struct blender_rgba : conv_rgba_pre<ColorT, Order>
    {
        ...
        // compositing function. Since the render buffer is in fact
        // premultiplied we omit the initial premultiplication and final
        // demultiplication.
    ...
    //======================================================blender_rgba_plain
    // Blends "plain" (non-premultiplied) colors into a plain
    // (non-premultiplied) buffer.
    template <class ColorT, class Order>
    ...
    struct comp_op_rgba_overlay : blender_base<ColorT, Order>
    {
        ...
        // otherwise
        //   Dca' = Sa.Da - 2.(Da - Dca).(Sa - Sca) + Sca.(1 - Da) + Dca.(1 -
        //   Sa)
        //
        // Da' = Sa + Da - Sa.Da
        static AGG_INLINE double calc(double dca,
            double sca,
            double da,
            double sa,
            double sada,
            ...
            double s1a)
        {
            return (2 * dca <= da) ? 2 * sca * dca + sca * d1a + dca * s1a
                                   : sada - 2 * (da - dca) * (sa - sca) +
                                         sca * d1a + dca * s1a;
        }
    ...
    struct comp_op_rgba_color_dodge : blender_base<ColorT, Order>
    {
        ...
        // otherwise if Sca < Sa
        //     Dca' = Sa.Da.min(1, Dca/Da.Sa/(Sa - Sca)) + Sca.(1 - Da) + Dca.(1
        //     - Sa)
        //
        // Da'  = Sa + Da - Sa.Da
        static AGG_INLINE double calc(double dca,
        ...
        {
            if (sca < sa)
                return sada * sd_min(1.0, (dca / da) * sa / (sa - sca)) +
                       sca * d1a + dca * s1a;
            if (dca > 0)
    ...
    struct comp_op_rgba_color_burn : blender_base<ColorT, Order>
    {
        ...
        {
            if (sca > 0)
                return sada * (1 - sd_min(1.0, (1 - dca / da) * sa / sca)) +
                       sca * d1a + dca * s1a;
            if (dca > da)
                return sada + dca * s1a;
            return dca * s1a;
    ...
    struct comp_op_rgba_hard_light : blender_base<ColorT, Order>
    {
        ...
        static AGG_INLINE double calc(double dca,
            ...
            double s1a)
        {
            return (2 * sca < sa) ? 2 * sca * dca + sca * d1a + dca * s1a
                                  : sada - 2 * (da - dca) * (sa - sca) +
                                        sca * d1a + dca * s1a;
        }
    ...
    struct comp_op_rgba_soft_light : blender_base<ColorT, Order>
    {
        ...
        // otherwise if 2.Sca > Sa and 4.Dca > Da
        //   Dca' = Dca.Sa + (2.Sca.Da - Sa.Da).((Dca.Sa)^0.5 - Dca.Sa) + Sca.(1
        //   - Da) + Dca.(1 - Sa)
        //
        // Da'  = Sa + Da - Sa.Da
        static AGG_INLINE double calc(double dca,
            ...
            double s1a)
        {
            double dcasa = dca * sa;
            if (2 * sca <= sa)
                return dcasa - (sada - 2 * sca * da) * dcasa * (sada - dcasa) +
                       sca * d1a + dca * s1a;
            if (4 * dca <= da)
                return dcasa +
                       (2 * sca * da - sada) *
                           ((((16 * dcasa - 12) * dcasa + 4) * dca * da) -
                               dca * da) +
                       sca * d1a + dca * s1a;
            return dcasa + (2 * sca * da - sada) * (std::sqrt(dcasa) - dcasa) +
                   sca * d1a + dca * s1a;
        }
    ...
    struct comp_op_rgba_exclusion : blender_base<ColorT, Order>
    {
            ...
            cover_type cover)
        {
            ...
            if (s.a > 0)
            {
                ...
                double d1a = 1 - d.a;
                double s1a = 1 - s.a;
                d.r = (s.r * d.a + d.r * s.a - 2 * s.r * d.r) + s.r * d1a +
                      d.r * s1a;
                d.g = (s.g * d.a + d.g * s.a - 2 * s.g * d.g) + s.g * d1a +
                      d.g * s1a;
                d.b = (s.b * d.a + d.b * s.a - 2 * s.b * d.b) + s.b * d1a +
                      d.b * s1a;
                d.a += s.a - s.a * d.a;
                set(p, clip(d));
            }
    ...
    struct comp_op_adaptor_rgba_plain
    {
            ...
            cover_type cover)
        {
            ...
            comp_op_adaptor_rgba<ColorT, Order>::blend_pix(
                op, p, r, g, b, a, cover);
            multiplier_rgba<ColorT, Order>::demultiply(p);
        }
    };
    ...
    struct comp_op_adaptor_clip_to_dst_rgba_plain
    {
        ...
        {
            multiplier_rgba<ColorT, Order>::premultiply(p);
            comp_op_adaptor_clip_to_dst_rgba<ColorT, Order>::blend_pix(
                op, p, r, g, b, a, cover);
            multiplier_rgba<ColorT, Order>::demultiply(p);
        }
    };
    ...
    struct comp_adaptor_rgba
    {
        ...
        static AGG_INLINE void blend_pix(unsigned,
            value_type* p,
            value_type r,
            value_type g,
            value_type b,
    ...
    struct comp_adaptor_clip_to_dst_rgba_plain
    {
        ...
        {
            multiplier_rgba<color_type, order_type>::premultiply(p);
            comp_adaptor_clip_to_dst_rgba<BlenderPre>::blend_pix(
                op, p, r, g, b, a, cover);
            multiplier_rgba<color_type, order_type>::demultiply(p);
        }
    };
    ...
    public:
        ...
        struct pixel_type
        {
            ...
            {
                return color_type(c[order_type::R],
                    c[order_type::G],
                    c[order_type::B],
                    c[order_type::A]);
            }
    ...
    public:
        ...
        {
            return (pixel_type*)(m_rbuf->row_ptr(x, y, len) +
        ...
        AGG_INLINE const pixel_type* pix_value_ptr(int x, int y) const
        {
            ...
            return p ? (pixel_type*)(p + sizeof(value_type) * (x * pix_step))
                     : 0;
        }
        ...
        {
            for_each_pixel(
                multiplier_rgba<color_type, order_type>::premultiply);
        }
        ...
        {
            for_each_pixel(
                apply_gamma_dir_rgba<color_type, order_type, GammaLut>(g));
        }
        ...
        {
            for_each_pixel(
                apply_gamma_inv_rgba<color_type, order_type, GammaLut>(g));
        }
    ...
    private:
        //--------------------------------------------------------------------
        AGG_INLINE void blend_pix(
            pixel_type* p, const color_type& c, unsigned cover = cover_full)
        {
    ...
    //-----------------------------------------------------------------------
    typedef pixfmt_alpha_blend_rgba<blender_rgba32, rendering_buffer>
        pixfmt_rgba32;
    typedef pixfmt_alpha_blend_rgba<blender_argb32, rendering_buffer>
        pixfmt_argb32;
    typedef pixfmt_alpha_blend_rgba<blender_abgr32, rendering_buffer>
        pixfmt_abgr32;
    typedef pixfmt_alpha_blend_rgba<blender_bgra32, rendering_buffer>
        pixfmt_bgra32;

    typedef pixfmt_alpha_blend_rgba<blender_srgba32, rendering_buffer>
        pixfmt_srgba32;
    typedef pixfmt_alpha_blend_rgba<blender_sargb32, rendering_buffer>
        pixfmt_sargb32;
    typedef pixfmt_alpha_blend_rgba<blender_sabgr32, rendering_buffer>
        pixfmt_sabgr32;
    typedef pixfmt_alpha_blend_rgba<blender_sbgra32, rendering_buffer>
        pixfmt_sbgra32;
    ...
    typedef pixfmt_alpha_blend_rgba<blender_srgba32_pre, rendering_buffer>
        pixfmt_srgba32_pre;
    typedef pixfmt_alpha_blend_rgba<blender_sargb32_pre, rendering_buffer>
        pixfmt_sargb32_pre;
    typedef pixfmt_alpha_blend_rgba<blender_sabgr32_pre, rendering_buffer>
        pixfmt_sabgr32_pre;
    typedef pixfmt_alpha_blend_rgba<blender_sbgra32_pre, rendering_buffer>
        pixfmt_sbgra32_pre;
    ...
    typedef pixfmt_alpha_blend_rgba<blender_srgba32_plain, rendering_buffer>
        pixfmt_srgba32_plain;
    typedef pixfmt_alpha_blend_rgba<blender_sargb32_plain, rendering_buffer>
        pixfmt_sargb32_plain;
    typedef pixfmt_alpha_blend_rgba<blender_sabgr32_plain, rendering_buffer>
        pixfmt_sabgr32_plain;
    typedef pixfmt_alpha_blend_rgba<blender_sbgra32_plain, rendering_buffer>
        pixfmt_sbgra32_plain;

    typedef pixfmt_alpha_blend_rgba<blender_rgba64, rendering_buffer>
        pixfmt_rgba64;
    typedef pixfmt_alpha_blend_rgba<blender_argb64, rendering_buffer>
        pixfmt_argb64;
    typedef pixfmt_alpha_blend_rgba<blender_abgr64, rendering_buffer>
        pixfmt_abgr64;
    typedef pixfmt_alpha_blend_rgba<blender_bgra64, rendering_buffer>
        pixfmt_bgra64;
    ...
    typedef pixfmt_alpha_blend_rgba<blender_rgba64_plain, rendering_buffer>
        pixfmt_rgba64_plain;
    typedef pixfmt_alpha_blend_rgba<blender_argb64_plain, rendering_buffer>
        pixfmt_argb64_plain;
    typedef pixfmt_alpha_blend_rgba<blender_abgr64_plain, rendering_buffer>
        pixfmt_abgr64_plain;
    typedef pixfmt_alpha_blend_rgba<blender_bgra64_plain, rendering_buffer>
        pixfmt_bgra64_plain;

    typedef pixfmt_alpha_blend_rgba<blender_rgba128, rendering_buffer>
        pixfmt_rgba128;
    typedef pixfmt_alpha_blend_rgba<blender_argb128, rendering_buffer>
        pixfmt_argb128;
    typedef pixfmt_alpha_blend_rgba<blender_abgr128, rendering_buffer>
        pixfmt_abgr128;
    typedef pixfmt_alpha_blend_rgba<blender_bgra128, rendering_buffer>
        pixfmt_bgra128;

    typedef pixfmt_alpha_blend_rgba<blender_rgba128_pre, rendering_buffer>
        pixfmt_rgba128_pre;
    typedef pixfmt_alpha_blend_rgba<blender_argb128_pre, rendering_buffer>
        pixfmt_argb128_pre;
    typedef pixfmt_alpha_blend_rgba<blender_abgr128_pre, rendering_buffer>
        pixfmt_abgr128_pre;
    typedef pixfmt_alpha_blend_rgba<blender_bgra128_pre, rendering_buffer>
        pixfmt_bgra128_pre;

    typedef pixfmt_alpha_blend_rgba<blender_rgba128_plain, rendering_buffer>
        pixfmt_rgba128_plain;
    typedef pixfmt_alpha_blend_rgba<blender_argb128_plain, rendering_buffer>
        pixfmt_argb128_plain;
    typedef pixfmt_alpha_blend_rgba<blender_abgr128_plain, rendering_buffer>
        pixfmt_abgr128_plain;
    typedef pixfmt_alpha_blend_rgba<blender_bgra128_plain, rendering_buffer>
        pixfmt_bgra128_plain;
