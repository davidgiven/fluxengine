...
namespace agg
{
    ...
    public:
        ...
        void begin(double x, double y, unsigned len)
        {
            ...
            dx -= x;
            dy -= y;
            int sx1 = uround(subpixel_scale / std::sqrt(dx * dx + dy * dy)) >>
                      subpixel_shift;
            dx = xt;
            dy = yt + delta;
            m_trans_inv.transform(&dx, &dy);
            dx -= x;
            dy -= y;
            int sy1 = uround(subpixel_scale / std::sqrt(dx * dx + dy * dy)) >>
                      subpixel_shift;
            ...
            dx -= x;
            dy -= y;
            int sx2 = uround(subpixel_scale / std::sqrt(dx * dx + dy * dy)) >>
                      subpixel_shift;
            dx = xt;
            dy = yt + delta;
            m_trans_inv.transform(&dx, &dy);
            dx -= x;
            dy -= y;
            int sy2 = uround(subpixel_scale / std::sqrt(dx * dx + dy * dy)) >>
                      subpixel_shift;
