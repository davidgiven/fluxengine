...
namespace agg
{
    ...
    public:
        ...
        int calculate(int x, int y, int d) const
        {
            if (m_buffer)
            {
                ...
                return iround(m_buffer[py * m_width + px] * (m_d2 / 256) + m_d1)
    ...
    static AGG_INLINE int square(int x)
    {
        return x * x;
    }
            ...
            if (agg::bounding_rect_single(conv, 0, &x1, &y1, &x2, &y2))
            {
                ...
                if (buffer)
                {
                    ...
                    agg::renderer_primitives<
                        agg::renderer_base<agg::pixfmt_gray8>>
                        prim(renb);
                    agg::rasterizer_outline<renderer_primitives<
                        agg::renderer_base<agg::pixfmt_gray8>>>
                        ras(prim);

                    agg::trans_affine mtx;
                    mtx *= agg::trans_affine_translation(
                        -x1 + m_frame, -y1 + m_frame);
