...
namespace agg
{
    ...
    void color_conv(RenBuf* dst, const RenBuf* src, CopyRow copy_row_functor)
    {
        ...
        if (width)
        {
            ...
            {
                copy_row_functor(
                    dst->row_ptr(0, y, width), src->row_ptr(y), width);
            }
        }
    }
    ...
    void color_conv_row(
        int8u* dst, const int8u* src, unsigned width, CopyRow copy_row_functor)
    {
    ...
    struct conv_pixel
    {
        ...
        {
            // Read a pixel from the source format and write it to the
            // destination format.
    ...
    // Specialization for case where source and destination formats are
    // identical.
