...
namespace agg
{
    ...
    //                                      // of the current row
    //
    // unsigned num_spans = sl.num_spans(); // Number of spans. It's guaranteed
    // that
    //                                      // num_spans is always greater than
    //                                      0.
    //
    // do
    // {
    //     const scanline_u8::cover_type* covers =
    //         span->covers;                     // The array of the cover
    //         values
    //
    //     int num_pix = span->len;              // Number of pixels of the
    //     span.
    ...
    //     **************************************
    //
    //     unsigned char* dst = row + x;  // Calculate the start address of the
    //     row.
    ...
    public:
        ...
        void finalize(int span_y)
        {
            ...
            if (m_alpha_mask)
            {
                ...
                do
                {
                    m_alpha_mask->combine_hspan(
                        span->x, base_type::y(), span->covers, span->len);
                    ++span;
                } while (--count);
            }
        }
    ...
    public:
        ...
        void add_cells(int x, unsigned len, const cover_type* covers)
        {
            ...
            {
                m_spans.add(span(
                    coord_type(x + m_min_x), coord_type(len), &m_covers[x]));
            }
            m_last_x = x + len - 1;
        }
