...
namespace agg
{
    ...
    struct gray8T
    {
        ...
        static value_type luminance(const rgba& c)
        {
            ...
            return value_type(uround(
                (0.2126 * c.r + 0.7152 * c.g + 0.0722 * c.b) * base_mask));
        }
        ...
        //--------------------------------------------------------------------
        gray8T(const self_type& c, unsigned a_): v(c.v), a(value_type(a_)) {}
        ...
        T convert_from_sRGB() const
        {
            typename T::value_type y =
                sRGB_conv<typename T::value_type>::rgb_from_sRGB(v);
            return T(
                y, y, y, sRGB_conv<typename T::value_type>::alpha_from_sRGB(a));
        }
        ...
        {
            typename T::value_type y =
                sRGB_conv<typename T::value_type>::rgb_to_sRGB(v);
            return T(
                y, y, y, sRGB_conv<typename T::value_type>::alpha_to_sRGB(a));
        }
        ...
        static AGG_INLINE value_type prelerp(
            value_type p, value_type q, value_type a)
        {
        ...
        static AGG_INLINE value_type lerp(
            value_type p, value_type q, value_type a)
        {
        ...
        self_type& demultiply()
        {
            if (a < base_mask)
            {
                ...
                else
                {
                    ...
                    v = value_type(
                        (v_ > base_mask) ? (value_type)base_mask : v_);
                }
            }
            return *this;
        }
        ...
        AGG_INLINE void add(const self_type& c, unsigned cover)
        {
            ...
            v = (value_type)((cv > calc_type(base_mask)) ? calc_type(base_mask)
                                                         : cv);
            a = (value_type)((ca > calc_type(base_mask)) ? calc_type(base_mask)
                                                         : ca);
        }
    ...
    struct gray16
    {
        ...
        static value_type luminance(const rgba16& c)
        {
            // Calculate grayscale value as per ITU-R BT.709.
            return value_type(
                (13933u * c.r + 46872u * c.g + 4732u * c.b) >> 16);
        }
        ...
        //--------------------------------------------------------------------
        gray16(const self_type& c, unsigned a_): v(c.v), a(value_type(a_)) {}
        ...
        //--------------------------------------------------------------------
        gray16(const rgba8& c): v(luminance(c)), a((value_type(c.a) << 8) | c.a)
        {
        }
        ...
        //--------------------------------------------------------------------
        gray16(const rgba16& c): v(luminance(c)), a(c.a) {}
        ...
        {
            return sgray8(sRGB_conv<value_type>::rgb_to_sRGB(v),
        ...
        self_type& demultiply()
        {
            if (a < base_mask)
            {
                ...
                {
                    calc_type v_ = (calc_type(v) * base_mask) / a;
                    v = (v_ > base_mask) ? value_type(base_mask)
                                         : value_type(v_);
                }
            }
            return *this;
    ...
    struct gray32
    {
        ...
        //--------------------------------------------------------------------
        explicit gray32(value_type v_, value_type a_ = 1): v(v_), a(a_) {}

        //--------------------------------------------------------------------
        gray32(const self_type& c, value_type a_): v(c.v), a(a_) {}

        //--------------------------------------------------------------------
        gray32(const rgba& c): v(luminance(c)), a(value_type(c.a)) {}

        //--------------------------------------------------------------------
        gray32(const rgba8& c): v(luminance(c)), a(value_type(c.a / 255.0)) {}
        ...
        //--------------------------------------------------------------------
        gray32(const rgba16& c): v(luminance(c)), a(value_type(c.a / 65535.0))
        {
        }

        //--------------------------------------------------------------------
        gray32(const rgba32& c): v(luminance(c)), a(value_type(c.a)) {}
        ...
        static AGG_INLINE value_type lerp(
            value_type p, value_type q, value_type a)
        {
            // The form "p + a * (q - p)" avoids a multiplication, but may
            // produce an inaccurate result. For example, "p + (q - p)" may not
            // be exactly equal to q. Therefore, stick to the basic expression,
            // which at least produces the correct result at either extreme.
            return (1 - a) * p + a * q;
        }
        ...
        {
            return self_type(
                value_type(v + (c.v - v) * k), value_type(a + (c.a - a) * k));
        }
