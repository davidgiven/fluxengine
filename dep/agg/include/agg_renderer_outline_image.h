...
namespace agg
{
    ...
    public:
        ...
        color_type pixel(int x, int y) const
        {
            if (m_scale < 1.0)
            {
                ...
                int y2 = y1 + 1;
                rgba pix1 =
                    (y1 < 0) ? rgba::no_color() : rgba(m_source.pixel(x, y1));
                rgba pix2 =
                    (y2 > h) ? rgba::no_color() : rgba(m_source.pixel(x, y2));
            ...
            else
            {
                ...
                if (y1 >= 0)
                    c += rgba(m_source.pixel(x, y1)) *= y1 + 1 - src_y1;
                while (++y1 < y2)
                {
                    if (y1 <= h)
                        c += m_source.pixel(x, y1);
                }
                if (y2 <= h)
                    c += rgba(m_source.pixel(x, y2)) *= src_y2 - y2;
    ...
    private:
        ...
        const line_image_scale<Source>& operator=(
            const line_image_scale<Source>&);
    ...
    public:
        ...
        void create(const Source& src)
        {
            ...
            m_offset_y_hr =
                m_dilation_hr + m_half_height_hr - line_subpixel_scale / 2;
            m_half_height_hr += line_subpixel_scale / 2;

            m_data.resize(
                (m_width + m_dilation * 2) * (m_height + m_dilation * 2));
    ...
    public:
        ...
        distance_interpolator4(int x1,
            ...
            m_dist_end((line_mr(x + line_subpixel_scale / 2) - line_mr(ex)) *
                           m_dy_end -
            ...
            m_len(uround(len / scale))
        {
            ...
            m_dy_pict = dx;
            m_dist_pict =
                ((x + line_subpixel_scale / 2 - (x1 - dy)) * m_dy_pict -
    ...
    public:
        ...
        line_interpolator_image(renderer_type& ren,
            ...
            m_max_extent(
                (m_width + line_subpixel_scale) >> line_subpixel_shift),
    ...
    public:
        ...
        //-------------------------------------------------------------------------
        void line3_no_clip(
            const line_parameters& lp, int sx, int sy, int ex, int ey)
        {
            ...
            line_interpolator_image<self_type> li(
                *this, lp, sx, sy, ex, ey, m_start, m_scale_x);
            if (li.vertical())
            {
                while (li.step_ver())
                    ;
            ...
            if (m_clipping)
            {
                ...
                if ((flags & 4) == 0)
                {
                    if (flags)
                    {
                        ...
                        {
                            m_start +=
                                uround(calc_distance(lp.x1, lp.y1, x1, y1) /
                                       m_scale_x);
                            sx = x1 + (y2 - y1);
                            sy = y1 - (x2 - x1);
                        }
