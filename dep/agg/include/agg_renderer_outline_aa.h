...
namespace agg
{
    ...
    public:
        ...
        distance_interpolator2() {}
        distance_interpolator2(
            int x1, int y1, int x2, int y2, int sx, int sy, int x, int y):
            m_dx(x2 - x1),
            m_dy(y2 - y1),
            ...
            m_dist(iround(
                double(x + line_subpixel_scale / 2 - x2) * double(m_dy) -
                double(y + line_subpixel_scale / 2 - y2) * double(m_dx))),

            m_dist_start((line_mr(x + line_subpixel_scale / 2) - line_mr(sx)) *
                             m_dy_start -
                         (line_mr(y + line_subpixel_scale / 2) - line_mr(sy)) *
                             m_dx_start)
        {
            m_dx <<= line_subpixel_shift;
            m_dy <<= line_subpixel_shift;
        ...
        distance_interpolator2(
            int x1, int y1, int x2, int y2, int ex, int ey, int x, int y, int):
            m_dx(x2 - x1),
            m_dy(y2 - y1),
            ...
            m_dist_start((line_mr(x + line_subpixel_scale / 2) - line_mr(ex)) *
                             m_dy_start -
                         (line_mr(y + line_subpixel_scale / 2) - line_mr(ey)) *
                             m_dx_start)
        {
            m_dx <<= line_subpixel_shift;
            m_dy <<= line_subpixel_shift;
    ...
    public:
        ...
        distance_interpolator3(int x1,
            ...
            m_dist_end(
                (line_mr(x + line_subpixel_scale / 2) - line_mr(ex)) *
                    m_dy_end -
    ...
    public:
        ...
        line_interpolator_aa_base(renderer_type& ren, line_parameters& lp):
            ...
            m_old_y(m_y),
            m_count(
                (lp.vertical ? std::abs((lp.y2 >> line_subpixel_shift) - m_y)
    ...
    public:
        ...
        bool step_hor()
        {
            ...
            cover_type* p0 =
                base_type::m_covers + base_type::max_half_width + 2;
            cover_type* p1 = p0;
            ...
            *p1++ = (cover_type)base_type::m_ren.cover(s1);
            ...
            base_type::m_ren.blend_solid_vspan(
                base_type::m_x, base_type::m_y - dy + 1, unsigned(p1 - p0), p0);
        ...
        bool step_ver()
        {
            ...
            *p1++ = (cover_type)base_type::m_ren.cover(s1);
            ...
            base_type::m_ren.blend_solid_hspan(
                base_type::m_x - dx + 1, base_type::m_y, unsigned(p1 - p0), p0);
            return ++base_type::m_step < base_type::m_count;
        }
    ...
    public:
        ...
        //---------------------------------------------------------------------
        line_interpolator_aa1(
            renderer_type& ren, line_parameters& lp, int sx, int sy):
            line_interpolator_aa_base<Renderer>(ren, lp),
            m_di(lp.x1,
                lp.y1,
                lp.x2,
                ...
                lp.y1 & ~line_subpixel_mask)
        {
            ...
            if (lp.vertical)
            {
                do
                {
                    ...
                    base_type::m_x =
                        (base_type::m_lp->x1 + base_type::m_li.y()) >>
                        line_subpixel_shift;
            ...
            else
            {
                do
                {
                    ...
                    base_type::m_y =
                        (base_type::m_lp->y1 + base_type::m_li.y()) >>
                        line_subpixel_shift;
    ...
    public:
        ...
        //---------------------------------------------------------------------
        line_interpolator_aa2(
            renderer_type& ren, line_parameters& lp, int ex, int ey):
            line_interpolator_aa_base<Renderer>(ren, lp),
            m_di(lp.x1,
                lp.y1,
                lp.x2,
    ...
    public:
        ...
        template <class Cmp>
        void semidot_hline(
            Cmp cmp, int xc1, int yc1, int xc2, int yc2, int x1, int y1, int x2)
        {
            cover_type covers
                [line_interpolator_aa_base<self_type>::max_half_width * 2 + 4];
        ...
        void semidot(Cmp cmp, int xc1, int yc1, int xc2, int yc2)
        {
            ...
            int r = ((subpixel_width() + line_subpixel_mask) >>
                     line_subpixel_shift);
            if (r < 1)
                r = 1;
            ...
            do
            {
                ...
                {
                    semidot_hline(
                        cmp, xc1, yc1, xc2, yc2, x - dx0, y + dy0, x + dx0);
                    semidot_hline(
                        cmp, xc1, yc1, xc2, yc2, x - dx0, y - dy0, x + dx0);
                }
                dx0 = dx;
        ...
        void pie(int xc, int yc, int x1, int y1, int x2, int y2)
        {
            ...
            do
            {
                ...
                {
                    pie_hline(
                        xc, yc, x1, y1, x2, y2, x - dx0, y + dy0, x + dx0);
                    pie_hline(
                        xc, yc, x1, y1, x2, y2, x - dx0, y - dy0, x + dx0);
                }
                dx0 = dx;
        ...
        void line0(line_parameters& lp)
        {
            if (m_clipping)
            {
                ...
                int y2 = lp.y2;
                unsigned flags =
                    clip_line_segment(&x1, &y1, &x2, &y2, m_clip_box);
                if ((flags & 4) == 0)
                {
                    if (flags)
                    {
                        line_parameters lp2(x1,
                            y1,
                            x2,
                            y2,
            ...
            if (m_clipping)
            {
                ...
                if ((flags & 4) == 0)
                {
                    if (flags)
                    {
                        ...
                        {
                            while (std::abs(sx - lp.x1) + std::abs(sy - lp.y1) >
                                   lp2.len)
                            {
                                sx = (lp.x1 + sx) >> 1;
                                sy = (lp.y1 + sy) >> 1;
            ...
            if (m_clipping)
            {
                ...
                if ((flags & 4) == 0)
                {
                    if (flags)
                    {
                        ...
                        {
                            while (std::abs(ex - lp.x2) + std::abs(ey - lp.y2) >
                                   lp2.len)
                            {
                                ex = (lp.x2 + ex) >> 1;
                                ey = (lp.y2 + ey) >> 1;
        ...
        void line3_no_clip(line_parameters& lp, int sx, int sy, int ex, int ey)
        {
            if (lp.len > line_max_length)
            {
                ...
                line3_no_clip(
                    lp1, (lp.x1 + sx) >> 1, (lp.y1 + sy) >> 1, mx, my);
                line3_no_clip(
                    lp2, mx, my, (lp.x2 + ex) >> 1, (lp.y2 + ey) >> 1);
                return;
            }
