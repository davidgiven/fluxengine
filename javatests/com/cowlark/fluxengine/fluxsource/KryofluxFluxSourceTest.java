...
public class KryofluxFluxSourceTest
{
    ...
    public void readsSingleFluxFromDirectory() throws Exception
    {
        ...
        KryofluxFluxSourceProto config = KryofluxFluxSourceProto.newBuilder()
                .setDirectory(dir.toString())
                .build();
        KryofluxFluxSource source = new KryofluxFluxSource(config);

        assertThat(source.readSingleFlux(80, 0).rawBytes().toByteArray())
                .isEqualTo(new byte[]{(byte) 0x8f});
        ...
        String sorted = Arrays.stream(tracks.split(" ")).sorted()
